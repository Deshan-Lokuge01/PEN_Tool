...
void WiFiScan::RunBeaconScan(uint8_t scan_mode, uint16_t color)
{
  if (scan_mode == WIFI_SCAN_AP)
    startPcap("beacon");
      ...
      if (gps_obj.getGpsModuleStatus()) {
        startLog("wardrive");
        String header_line = "WigleWifi-1.4,appRelease=" + (String)PENETRATION_VERSION + ",model=ESP32 PENETRATION,release=" + (String)PENETRATION_VERSION + ",device=ESP32 PENETRATION,display=SPI TFT,board=ESP32 PENETRATION,brand=JustCallMeKoko\nMAC,SSID,AuthMode,FirstSeen,Channel,RSSI,CurrentLatitude,CurrentLongitude,AltitudeMeters,AccuracyMeters,Type\n";
