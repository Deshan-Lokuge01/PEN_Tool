...
void CommandLine::RunSetup() {
  ...
  Serial.println(F("\n\n--------------------------------\n"));
  Serial.println(F("         ESP32 PENTool      \n"));
  Serial.println("            " + version_number + "\n");
  Serial.println(F("       By: 23RD Batch\n"));
  Serial.println(F("--------------------------------\n\n"));
...
void CommandLine::runCommand(String input) {
  ...
  if (!wifi_scan_obj.scanning()) {
    ...
    // Deauth sniff
      ...
      Serial.println("Starting Deauthentication sniff. Stop with " + (String)STOPSCAN_CMD);
      #ifdef HAS_SCREEN
        display_obj.clearScreen();
        menu_function_obj.drawStatusBar();
      #endif
    ...
    if (cmd_args.get(0) == UPDATE_CMD) {
      ...
      // Update via OTA
      //if (w_sw != -1) {
      //  Serial.println("Starting PENETRATION OTA Update. Stop with " + (String)STOPSCAN_CMD);
